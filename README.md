...
apps/
  ...
  api/            NestJS — thin controllers -> services
    src/app/
      auth/       Supabase JWT guard, auth context (@CurrentUser/@CurrentOrg), roles guard, password login
      ...
      ai/         natural-language -> structured offer draft (Gemini-only)
...
npm run db:seed        # seed the demo org, its test user (by authId), and the catalogue
npm run dev            # api (http://localhost:3000/api) + web (http://localhost:4200)
```

The UI defaults to **German**; switch to English on the account page. Sign in
with the email/password of a Supabase Auth user that is linked (via `authId`) to
a seeded app user, then try the assistant: *"Angebot für 100m² Hausabbruch"*.
Set `SUPABASE_URL` / `SUPABASE_ANON_KEY` (backend password verification) and
`GEMINI_API_KEY` (the offer drafter) — see `.env.example`.

## How it works

- **Catalogue** is seeded from `scripts/baupreise-catalogue.json` (2,869 German
  trade items). A strict **+30% markup** is baked into the single `Item.price`
  at seed time (`round(base × 1.30, 2)`) — there is no separate baseline column.
...
- **AI assistant** sends the natural-language request to **Gemini**
  ...
  the org's catalogue to draft a structured, priced offer. Gemini is required:
  with no `GEMINI_API_KEY` drafting returns an error (no local fallback). Prices
  always come from the catalogue, never the model.
...
- **Auth** uses email/password sign-in verified **server-side**: `/auth/login`
  posts the credentials to Supabase Auth (GoTrue), then resolves the app user by
  `User.authId` (the Supabase UUID, kept separate from our own `User.id`). On
  success the API mints a Supabase-shaped HS256 JWT (verified with
  `SUPABASE_JWT_SECRET`) carrying `organisation_id` / `app_role` claims; the
  guard resolves the caller's org + role from the token (or provisions from the
  `User` table by `authId` on first sign-in).
